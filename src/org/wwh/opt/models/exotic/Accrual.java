...
public class Accrual extends JPanel
{
	public static final long serialVersionUID = 10040;
    ...
    double AccrualPricer(int i, double d, double d1, double d2, 
            double d3, double d4, double d5, double d6, double d7)
    {
        ...
        for(int j = 1; j <= k; j++)
        {
            double d10 = (Math.log(d / d1) + (d6 - (d7 * d7) / 2D) * (double)j * d4) / (d7 * Math.sqrt((double)j * d4));
            d9 += Common.CND((double)l * d10) * d4;
        }

        d11 = d2 * Math.exp(-d5 * d3) * d9 + d4 * d2 * Math.exp(-d5 * d3) * d8;
        return d11;
    }
    ...
    public boolean action(Event event, Object obj)
    {
        ...
        s = TextField_Interval.getText();
        double1 = Double.valueOf(s);
        Interval = double1.doubleValue();
        s = TextField_Credit.getText();
        double1 = Double.valueOf(s);
        Credit = double1.doubleValue();
        s = TextField_Vol.getText();
        double1 = Double.valueOf(s);
        v = double1.doubleValue() / 100D;
        int i = PutCall.getSelectedIndex();
        double d = AccrualPricer(i, S, X, Credit, T, Interval, r, r - D, v);
        DecimalFormat decimalformat = new DecimalFormat("0.#####");
        double d1 = 0.01D * S;
        double d2 = 0.01D * v;
        double d3 = (1.0D / d1) * (AccrualPricer(i, S + d1, X, Credit, T, Interval, r, r - D, v) - AccrualPricer(i, S, X, Credit, T, Interval, r, r - D, v));
        double d4 = (1.0D / (d1 * d1)) * ((AccrualPricer(i, S + d1, X, Credit, T, Interval, r, r - D, v) - 2D * AccrualPricer(i, S, X, Credit, T, Interval, r, r - D, v)) + AccrualPricer(i, S - d1, X, Credit, T, Interval, r, r - D, v));
        double d5 = (1.0D / d2) * (AccrualPricer(i, S, X, Credit, T, Interval, r, r - D, v + d2) - AccrualPricer(i, S, X, Credit, T, Interval, r, r - D, v));
    ...
    public void init()
    {
        ...
        gridbagconstraints.fill = 0;
        gridbagconstraints.anchor = 13;
        textLabel = new Label("Credit", 0);
        gridbaglayout.setConstraints(textLabel, gridbagconstraints);
        add(textLabel);
        buildConstraints(gridbagconstraints, 1, i, 1, 1, 0, 0);
        gridbagconstraints.fill = 2;
        TextField_Credit = new TextField(5);
        gridbaglayout.setConstraints(TextField_Credit, gridbagconstraints);
        add(TextField_Credit);
        TextField_Credit.setText("5");
        i++;
        buildConstraints(gridbagconstraints, 0, i, 1, 1, 0, 40);
        gridbagconstraints.fill = 0;
        gridbagconstraints.anchor = 13;
        textLabel = new Label("Time Interval", 0);
        gridbaglayout.setConstraints(textLabel, gridbagconstraints);
        add(textLabel);
        buildConstraints(gridbagconstraints, 1, i, 1, 1, 0, 0);
        gridbagconstraints.fill = 2;
        TextField_Interval = new TextField(5);
        gridbaglayout.setConstraints(TextField_Interval, gridbagconstraints);
        add(TextField_Interval);
        TextField_Interval.setText("0.00274");
        i++;
        buildConstraints(gridbagconstraints, 0, i, 1, 1, 0, 40);
        gridbagconstraints.fill = 0;
        gridbagconstraints.anchor = 13;
    ...
    TextField TextField_Credit;
    TextField TextField_Delta;
    TextField TextField_Vega;
    TextField TextField_Interval;
