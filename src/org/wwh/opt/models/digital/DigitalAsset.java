...
public class DigitalAsset extends JPanel
{
	public static final long serialVersionUID = 10030;
    ...
    double CashPricer(int i, double d, double d1, double d2, 
            double d3, double d4, double d5)
    {
        double d7 = 0.0D;
        double d6 = (Math.log(d / d1) + (d4 + (d5 * d5) / 2D) * d2) / (d5 * Math.sqrt(d2));
        if(i == 0)
            d7 = d * Math.exp((d4 - d3) * d2) * Common.CND(d6);
        else
        if(i == 1)
            d7 = d * Math.exp((d4 - d3) * d2) * Common.CND(-d6);
        return d7;
    ...
    public boolean action(Event event, Object obj)
    {
        ...
        double d = CashPricer(i, S, X, T, r, r - D, v);
