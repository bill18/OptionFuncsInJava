package org.wwh.opt.models.digital;
...
{
	public static final long serialVersionUID = 10031;
    ...
    double GapPricer(int i, double d, double d1, double d2, 
            double d3, double d4, double d5, double d6)
    {
        ...
        if(i == 0)
            d9 = d * Math.exp((d5 - d4) * d3) * Common.CND(d7) - d2 * Math.exp(-d4 * d3) * Common.CND(d8);
        else
        if(i == 1)
            d9 = d2 * Math.exp(-d4 * d3) * Common.CND(-d8) - d * Math.exp((d5 - d4) * d3) * Common.CND(-d7);
        return d9;
    }
    ...
    public boolean action(Event event, Object obj)
    {
        ...
        double d = GapPricer(i, S, X, X2, T, r, r - D, v);
